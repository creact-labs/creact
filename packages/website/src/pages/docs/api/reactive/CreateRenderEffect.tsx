...
const CreateRenderEffect: Component = () => {
  return (
    <>
      ...
      <p class="docs-description">Runs a function synchronously during render instead of queuing it. Used internally by the runtime; rarely needed in application code.</p>
      ...
      <table>
        ...
        <tbody>
          <tr><td><code>fn</code></td><td><code>(v: Init | Next) =&gt; Next</code></td><td>Effect function. Runs immediately during render.</td></tr>
      ...
      <DocCodeBlock code={`createRenderEffect(() => {
  ...
  console.log('Render-phase:', value());
      ...
      <Callout type="info">
        <p>
          Render effects run before user effects (<code>createEffect</code>). Use this when
          you need synchronous reactions during the render phase, such as component render tracking.
        </p>
      </Callout>
    </>
  );
