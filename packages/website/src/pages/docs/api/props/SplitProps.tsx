...
const SplitProps: Component = () => {
  return (
    <>
      <h1>splitProps</h1>
      <p class="docs-description">Splits a props object into multiple groups by key. Preserves reactivity.</p>

      <DocCodeBlock code={`const [local, others] = splitProps(props, ['class', 'style']);`} />
      ...
      />
      <p>
        Takes the props object followed by one or more arrays of keys. Returns a tuple with one
        object per key group plus a remainder object containing all unmatched keys.
      </p>

      <DocHeading level={2} id="usage">Usage</DocHeading>
      <DocCodeBlock code={`function Server(props: { port: number; host?: string; handler: () => void }) {
