...
Dir.glob("#{config_dir}/*.json").each do |file|
  ...
  begin
    ...
    if region == "us-east-1"
      ...
      s3.create_bucket(bucket: bucket, create_bucket_configuration: { location_constraint: region })
    end
  end

  # Enable versioning
  s3.put_bucket_versioning(bucket: bucket, versioning_configuration: { status: "Enabled" })
