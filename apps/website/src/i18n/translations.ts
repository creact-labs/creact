...
import getting_started_why_creact from "./resources/en/docs/getting_started/why_creact.json";
...
import guides_troubleshooting from "./resources/en/docs/guides/troubleshooting.json";
...
const en = {
  docs: {
    ...
    getting_started: {
      why_creact: getting_started_why_creact,
    ...
    guides: {
      ...
      troubleshooting: guides_troubleshooting,
...
// en is the hand-edited source and the fallback; its static tree above also
// derives the TranslationKey type. Every other locale is generated by the
// localization pipeline and loaded from disk.
export type Locale =
  | "en"
  | "de"
  | "es"
...
/** Every locale the site ships; en first as the source and fallback. */
...
// Load every locale's JSON files. A file at
// resources/<locale>/docs/getting_started/why_creact.json maps to the tree
// path docs.getting_started.why_creact, mirroring the en tree above.
const localeModules = import.meta.glob("./resources/*/**/*.json", {
  eager: true,
  import: "default",
}) as Record<string, unknown>;

function buildLocale(code: Locale): Resources {
  const prefix = `./resources/${code}/`;
  ...
  return tree as Resources;
}

/** The full resource tree per locale — exported for completeness tests */
export const resources: Record<Locale, Resources> = Object.fromEntries(
  LOCALES.map((code) => [code, code === "en" ? en : buildLocale(code)]),
) as Record<Locale, Resources>;

/**
 * Best available locale for a browser/OS language tag. Handles exact tags
 * (`pt-BR`), bare languages (`pt` → `pt-BR`, `fr-CA` → `fr`), and Chinese
 * script variants (`zh-TW`/`zh-Hant` → traditional, otherwise simplified).
 */
export function matchLocale(pref: string): Locale | undefined {
  const lower = pref.toLowerCase();
  const exact = LOCALES.find((l) => l.toLowerCase() === lower);
  if (exact) return exact;
  const base = lower.split("-")[0];
  if (base === "zh") {
    const traditional = lower.includes("hant") || lower.includes("tw") || lower.includes("hk");
...
/** The locale to start in, read from the browser's (OS-driven) preferences. */
export function detectLocale(): Locale {
  ...
  for (const pref of prefs) {
    const match = pref && matchLocale(pref);
...
function resolve(tree: Resources, key: string): unknown {
 ...
 * so text re-renders reactively when the locale changes. Falls back to en for
 * any key a generated locale is missing, then to the key itself.
 */
export function t(key: TranslationKey): string {
  const active = resolve(resources[locale()], key);
  if (typeof active === "string") return active;
  const fallback = resolve(resources.en, key);
