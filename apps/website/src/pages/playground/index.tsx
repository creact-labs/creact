...
import { createEditor } from "@/shared/playground/editor";
...
const Playground: Component = () => {
  ...
  function saveActive() {
    if (editor) contents[active()] = editor.state.doc.toString();
  }
  ...
  function openFile(name: string) {
    ...
    editor?.destroy();
    editor = undefined;
    if (isEditable(name)) editor = createEditor(editorEl, contents[name] ?? "");
  }
  ...
  onMount(() => {
    if (!meta()) return;
    editor = createEditor(editorEl, contents[ENTRY] ?? "");
    term = new Terminal({
      convertEol: true,
