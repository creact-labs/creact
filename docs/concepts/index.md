# What is CReact?

CReact is a declarative reactive runtime.

## Core Properties

### Declarative
Describe what you want, not how to get it.

### Universal
Works for AI agents, cloud infrastructure, APIs, webhooks, IoT devices.

### Reactive
When outputs change, dependent resources update automatically.

### Runtime
Runs continuously. Handles events, recovers from crashes.

## Next Steps

- [Thinking in CReact](./thinking-in-creact.md) - Learn the mental model
- [Constructs](./constructs.md) - Your building blocks
- [Components](./components.md) - Composing with JSX
- [Reactivity](./reactivity.md) - When things change
- [Providers](./providers.md) - Connecting to the real world
- [Backend](./backend.md) - Persisting state

## Try it

- [Agentic Chatbot Demo](https://github.com/creact-labs/creact-agentic-chatbot-example) — Build a chatbot with web search and browsing
