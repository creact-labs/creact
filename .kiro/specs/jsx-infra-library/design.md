...
class TerraformCloudProvider implements IIaCAdapter {
  ...
  mapOutputs(externalOutputs: any): Record<string, any> {
    return Object.entries(externalOutputs).reduce(
      (acc, [key, value]) => {
...
class ProviderRouter implements ICloudProvider {
  ...
  async initialize(): Promise<void> {
    ...
    await Promise.all(Array.from(this.providers.values()).map((p) => p.initialize?.()));
  }
...
function MonorepoRoot() {
  ...
  return (
    <>
      ...
      <CReactApp key="frontend" source="./apps/frontend" context={{ backendUrl }} />

      {/* Deploy worker */}
      <CReactApp key="worker" source="./apps/worker" />
    </>
  );
}
```
...
```typescript
// src/cli/plan.ts
export async function planCommand(options: { stack: string; json: boolean }) {
  const creact = new CReact({
  ...
  const changeSet = reconciler.reconcile(previous?.cloudDOM || [], current);
...
export async function devCommand(options: {
  ...
  watcher.on('change', async (path) => {
    ...
    try {
      ...
      const totalChanges =
        changeSet.creates.length + changeSet.updates.length + changeSet.deletes.length;
