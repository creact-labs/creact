// ./constructs/network/NetworkConstruct.ts
...
const SUBNETS = [
  { cidr: "10.0.1.0/24", azSuffix: "a", name: (env: string) => `escambo-subnet-${env}-a` },
  { cidr: "10.0.2.0/24", azSuffix: "b", name: (env: string) => `escambo-subnet-${env}-b` },
];
...
export class NetworkConstruct extends Construct {
  constructor(scope: Construct, id: string, props: NetworkConstructProps) {
    ...
    const serviceSg = new SecurityGroup(this, "service_sg", {
      vpcId: vpc.id,
      description: "Service Security Group (App + DB)",
      ingress: [
        {
