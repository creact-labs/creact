...
interface ExtendedBackendProvider<TState = any> extends IBackendProvider<TState> {
  ...
  checkLock?(stackName: string): Promise<{ holder: string; acquiredAt: number; ttl: number } | null>;
...
export class StateMachine {
   ...
   */
  private emit(event: StateMachineEvent, state: DeploymentState, metadata?: Record<string, any>): void {
  ...
  async resumeDeployment(stackName: string): Promise<{
    ...
    if (this.backendProvider.checkLock) {
      ...
      if (lockInfo) {
        ...
        if (lockAge < lockTTL) {
          throw new DeploymentError(
            `Cannot resume: lock held by ${lockInfo.holder}`,
            {
    ...
    if (!state.changeSet || !state.changeSet.deploymentOrder || state.changeSet.deploymentOrder.length === 0) {
      throw new DeploymentError(
  ...
  async getCheckpointInfo(stackName: string): Promise<{
    ...
    const percentComplete = totalResources > 0
      ? Math.round(((checkpoint + 1) / totalResources) * 100)
      : 0;
